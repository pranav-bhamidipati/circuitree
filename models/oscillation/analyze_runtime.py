...
def main(
    ...
    param_sets = np.array(
        [convert_uniform_to_params(u, _PARAM_RANGES) for u in uniform_samples]
    )
    ...
    with mp.Pool(n_threads, initializer=lambda: run_one_param_set(inputs[0])) as pool:
        results = []
    ...
    sampled_quantities = np.array(
        [_rescale(u, lo, hi) for u, (lo, hi) in zip(uniform_samples.T, _PARAM_RANGES)]
    )
    ...
    if save:
        fpath = Path(save_dir).joinpath(
            f"2023-06-13_random_sample_runtimes_AIcircuit_newranges.csv"
        )
...
if __name__ == "__main__":
    ...
    main(save_dir=save_dir, n_samples=1500, n_replicates=1, n_threads=None, save=False)
